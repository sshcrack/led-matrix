...
static Args parse_args(int argc, char* argv[])
{
    ...
    if (a.audio_profile != "balanced" && a.audio_profile != "bass" && a.audio_profile != "percussion" && a.audio_profile != "ambient")
    ...
    if (!audio_options.contains("bpm"))
        audio_options["bpm"] = a.audio_bpm;
    if (!audio_options.contains("profile"))
        audio_options["profile"] = a.audio_profile;
    return a;
}
...
struct VisualMetricAccumulator {
    ...
    int lit_fraction_max_frame = -1;
    double mean_luma_sum = 0.0;
    double temporal_change_sum = 0.0;
    double temporal_change_max = 0.0;
    int temporal_change_max_frame = -1;
    ...
    void add(const std::vector<uint8_t>& rgb, const std::vector<uint8_t>& previous)
    {
        ...
        for (size_t i = 0; i < pixels; ++i) {
            ...
            if (previous.size() == rgb.size()) {
                const int delta = std::abs(r - static_cast<int>(previous[i * 3])) + std::abs(g - static_cast<int>(previous[i * 3 + 1])) +
        ...
        if (lit_fraction > lit_fraction_max) {
            lit_fraction_max = lit_fraction;
            lit_fraction_max_frame = frames;
        }
        mean_luma_sum += luma / static_cast<double>(pixels);
        if (previous.size() == rgb.size()) {
            const double temporal_change = static_cast<double>(changed) / static_cast<double>(pixels);
            temporal_change_sum += temporal_change;
            if (temporal_change > temporal_change_max) {
                temporal_change_max = temporal_change;
                temporal_change_max_frame = frames;
            }
            ++temporal_samples;
        }
        ++frames;
    ...
    [[nodiscard]] nlohmann::json json() const
    {
        ...
        return {{"frames", frames},
                ...
                {"lit_fraction_max_frame", lit_fraction_max_frame},
                {"mean_luma_average", mean_luma_sum / frame_count},
                {"temporal_change_average", temporal_samples > 0 ? temporal_change_sum / static_cast<double>(temporal_samples) : 0.0},
                {"temporal_change_max", temporal_change_max},
                {"temporal_change_max_frame", temporal_change_max_frame}};
    }
};

static std::vector<uint8_t> capture_canvas(rgb_matrix::FrameCanvas* canvas, int w, int h)
{
...
// ---------------------------------------------------------------------------
static Magick::Image make_frame(const std::vector<uint8_t>& rgb, int w, int h, size_t delay_centiseconds)
{
    Magick::Image img(Magick::Geometry(static_cast<size_t>(w), static_cast<size_t>(h)), Magick::Color(0, 0, 0));
    img.modifyImage();

    Magick::PixelPacket* pixels = img.getPixels(0, 0, static_cast<size_t>(w), static_cast<size_t>(h));
    ...
    for (size_t i = 0; i < total; ++i) {
        ...
        pixels[i].red = static_cast<Quantum>(static_cast<unsigned long>(rgb[i * 3]) * MaxRGB / 255UL);
        pixels[i].green = static_cast<Quantum>(static_cast<unsigned long>(rgb[i * 3 + 1]) * MaxRGB / 255UL);
        pixels[i].blue = static_cast<Quantum>(static_cast<unsigned long>(rgb[i * 3 + 2]) * MaxRGB / 255UL);
...
int main(int argc, char* argv[])
{
    ...
    if (ec) {
        spdlog::error("Cannot create output directory '{}': {}", args.output_dir, ec.message());
        return 1;
    }
    ...
    rgb_matrix::EmulatorMatrix* matrix = rgb_matrix::EmulatorMatrix::Create(led_opts, emu_opts);
    if (!matrix) {
    ...
    if (wrappers.empty()) {
        spdlog::warn(
            "No scenes found. Make sure PLUGIN_DIR points to the "
    ...
    if (args.dump_manifest) {
        ...
        for (const auto& wrapper : wrappers) {
            ...
            const auto descriptor = default_scene->get_descriptor();
            ...
            manifest.push_back({
                ...
                {"category", default_scene->get_category()},
                {"descriptor", Scenes::descriptor_to_json(descriptor)},
                {"needs_desktop", needs_desktop},
                {"runtime_inputs", {{"required", runtime_spec.required}, {"optional", runtime_spec.optional}}},
                {"capabilities",
    ...
    const size_t frame_delay_cs = static_cast<size_t>(std::max(1, 100 / args.fps));  // centiseconds
    ...
    for (const auto& wrapper : wrappers) {
        ...
        for (const auto& input : preview_spec.inputs) {
            ...
            if (it == preview_providers.end()) {
                spdlog::warn("Skipping '{}': preview input '{}' has no registered provider.", scene_name, input);
        ...
        ++attempted;
        spdlog::info("Rendering preview for '{}' ({} frames @ {} fps)…", scene_name, args.total_frames, args.fps);
        ...
        try {
            ...
            for (const auto& prop : scene->get_properties()) prop->dump_to_json(default_props);
            for (const auto& [name, value] : preview_spec.property_overrides.items()) default_props[name] = value;
            ...
            struct ProviderSession {
                ...
                ~ProviderSession()
                {
                    for (auto it = started.rbegin(); it != started.rend(); ++it) (*it)->end();
                }
            } provider_session;
            ...
            for (int f = 0; f < args.total_frames; ++f) {
                ...
                for (const auto& provider : scene_preview_providers) provider->update(preview_frame);
                canvas->Clear();
                ...
                const auto rgb = capture_canvas(canvas, args.matrix_width, args.matrix_height);
                ...
                previous_rgb = rgb;
                frames.push_back(make_frame(rgb, args.matrix_width, args.matrix_height, frame_delay_cs));

                if (!keep_going) {
                    spdlog::debug("Scene '{}' stopped at frame {}/{}", scene_name, f + 1, args.total_frames);
            ...
            if (args.strict && frames.size() >= 6 && identical_tail_frames >= 5) {
                spdlog::warn("Scene '{}' stopped changing during its final {} frames.", scene_name, identical_tail_frames + 1);
                ++skipped;
            }
            ...
            spdlog::warn("Scene '{}' failed ({}); skipping — existing preview (if any) preserved.", scene_name, e.what());
            ++skipped;
        }
        catch (...) {
