...
public:
    ...
    explicit ScriptedScenesPacket(const std::vector<uint8_t> &data) : UdpPacket(0x03), frameData(data) {}
...
public:
    ...
    std::optional<std::unique_ptr<UdpPacket, void (*)(UdpPacket *)>> compute_next_packet(const std::string sceneName) override;
