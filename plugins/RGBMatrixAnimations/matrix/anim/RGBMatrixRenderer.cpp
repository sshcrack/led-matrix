    ...
    : gridWidth(width), gridHeight(height), maxBrightness(brightnessLimit), cubeMode(inCubeMode)
{
    ...
    palette = new RGB_color[maxColours];
    palette[0] = RGB_color{0, 0, 0};
    coloursDefined = 0;
...
RGB_color RGBMatrixRenderer::newRandomColour()
{
...
RGB_color RGBMatrixRenderer::blendColour(RGB_color start, RGB_color end, uint8_t step, uint8_t steps)
{
...
RGB_color RGBMatrixRenderer::getColor(uint16_t id)
{
...
uint16_t RGBMatrixRenderer::getColourId(RGB_color colour)
{
    uint16_t id = 0;
...
//    uint16_t pixels = 0;
    for(int y=0; y<gridHeight; y++) {
        for(int x=0; x<gridWidth; x++) {
            ...
            if (colcode) {
                setPixel(x, y, getColor(colcode));
//                pixels++;
            }
            else {
                setPixel(x, y, RGB_color{0, 0, 0});
            }
        }
    }
    showPixels();
...
void RGBMatrixRenderer::setPixelColour(uint16_t x, uint16_t y, RGB_color colour, bool persistent)
{
    if (persistent) {
...
void RGBMatrixRenderer::setPixelInstant(uint16_t x, uint16_t y, RGB_color colour)
{
    setPixel(x,y,colour);
}

void RGBMatrixRenderer::drawOctants(int xc, int yc, int x, int y, int yPrev, RGB_color colour, bool solid, bool persistent)
{
    if (x > 0) {
...
void RGBMatrixRenderer::drawCircle(int xc, int yc, int radius, RGB_color colour, bool solid, bool persistent)
{
    int r = radius;
    int x = 0, y = r, yPrev = y;
    int d = 3 - 2 * r;
