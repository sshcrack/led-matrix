...
public:
  ...
  [[nodiscard]] SceneCapabilities get_capabilities() const override {
    ...
    // Video frames are already decoded/rendered by the normal desktop plugin
    // and streamed to this lightweight Pi proxy. Offloading the proxy would
    // only add another network/process hop.
