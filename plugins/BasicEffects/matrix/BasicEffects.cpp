...
float accent_envelope(const PostProcessEffect& effect, float attack_fraction = 0.14f)
{
    const float progress = std::clamp(PostProcessingEffect::get_effect_progress(effect), 0.0f, 1.0f);
    const float attack = std::clamp(progress / std::max(0.02f, attack_fraction), 0.0f, 1.0f);
    const float attack_smooth = attack * attack * (3.0f - 2.0f * attack);
    const float release_progress = std::clamp((progress - attack_fraction) / std::max(0.02f, 1.0f - attack_fraction), 0.0f, 1.0f);
    const float release_smooth = release_progress * release_progress * (3.0f - 2.0f * release_progress);
    return std::max(0.0f, effect.intensity) * attack_smooth * (1.0f - release_smooth);
}
...
const rgb_matrix::Color& sample(const std::vector<rgb_matrix::Color>& scratch, int width, int height, int x, int y)
{
...
{
    return static_cast<uint8_t>(std::clamp(static_cast<float>(a) + (static_cast<float>(b) - static_cast<float>(a)) * t, 0.0f, 255.0f));
}
...
void FlashEffect::apply(FrameCanvas* canvas, const PostProcessEffect& effect)
{
    if (!canvas)
        return;
    // Keep the public effect name for API compatibility, but turn the old
    // full-frame white pop into a smooth hue-preserving luminance pulse.
    const float pulse = std::clamp(accent_envelope(effect, 0.18f), 0.0f, 1.5f);
    if (pulse < 0.005f)
        return;
    const float gain = 1.0f + pulse * 0.32f;
    ...
    for (int y = 0; y < canvas->height(); ++y) {
        for (int x = 0; x < canvas->width(); ++x) {
            ...
            if (r == 0 && g == 0 && b == 0)
                continue;
            canvas->SetPixel(x, y, std::min(255, static_cast<int>(static_cast<float>(r) * gain)),
                             std::min(255, static_cast<int>(static_cast<float>(g) * gain)),
                             std::min(255, static_cast<int>(static_cast<float>(b) * gain)));
        }
    }
}
...
void RotateEffect::apply(FrameCanvas* canvas, const PostProcessEffect& effect)
{
    ...
    if (std::abs(rotation_degrees) < 1.0f)
        return;
...
void GlowEffect::apply(FrameCanvas* canvas, const PostProcessEffect& effect)
{
    if (!canvas)
        return;
    const float amount = std::clamp(accent_envelope(effect), 0.0f, 1.5f);
    if (amount < 0.01f)
        return;
    const int width = canvas->width();
    const int height = canvas->height();
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            canvas->SetPixel(x, y, std::min(255, static_cast<int>(center.r + blur_r * gain)),
...
void RgbSplitEffect::apply(FrameCanvas* canvas, const PostProcessEffect& effect)
{
    if (!canvas)
        return;
    const float amount = std::clamp(accent_envelope(effect), 0.0f, 1.0f);
    if (amount < 0.01f)
        return;
    const int width = canvas->width();
    const int height = canvas->height();
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            canvas->SetPixel(x, y, mix_channel(base.r, red.r, amount), base.g, mix_channel(base.b, blue.b, amount));
        }
    }
}
...
void GlitchEffect::apply(FrameCanvas* canvas, const PostProcessEffect& effect)
{
    if (!canvas)
        return;
    const float amount = std::clamp(accent_envelope(effect, 0.20f), 0.0f, 1.0f);
    if (amount < 0.01f)
        return;
    const int width = canvas->width();
    const int height = canvas->height();
    ...
    for (int y = 0; y < height; ++y) {
        ...
        const int shift = shifted ? static_cast<int>(h % static_cast<uint32_t>(max_shift * 2 + 1)) - max_shift : 0;
...
void PixelateEffect::apply(FrameCanvas* canvas, const PostProcessEffect& effect)
{
    ...
    for (int by = 0; by < height; by += block) {
        for (int bx = 0; bx < width; bx += block) {
            ...
            for (int y = by; y < y_end; ++y)
                for (int x = bx; x < x_end; ++x) canvas->SetPixel(x, y, r, g, b);
        }
    }
}
...
void ShockwaveEffect::apply(FrameCanvas* canvas, const PostProcessEffect& effect)
{
    ...
    const float amount = std::clamp(accent_envelope(effect, 0.10f), 0.0f, 1.5f);
    if (amount < 0.01f)
        return;
    const int width = canvas->width();
    const int height = canvas->height();
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            canvas->SetPixel(x, y, std::min(255, static_cast<int>(color.r + lift * (255 - color.r))),
