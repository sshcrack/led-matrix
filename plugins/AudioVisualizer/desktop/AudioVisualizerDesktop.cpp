...
void AudioVisualizerDesktop::addConnectionSettings()
{
    ...
    static bool alwaysSendData = cfg.alwaysSendData;
    if (ImGui::Checkbox("Always trigger post processing effects", &alwaysSendData))
        cfg.alwaysSendData = alwaysSendData;
}
...
{
    if (sceneName != "audio_spectrum" && !cfg.alwaysSendData)
        return std::nullopt;
