...
constexpr std::array<std::pair<float, float>, 7> FeatureRanges{
    {{20.0f, 60.0f}, {60.0f, 140.0f}, {140.0f, 400.0f}, {400.0f, 2000.0f}, {2000.0f, 5000.0f}, {5000.0f, 12000.0f}, {12000.0f, 20000.0f}}};
...
MusicAnalyzer::MusicAnalyzer(AudioVisualizerConfig& config)
    : config_(config),
      fftInput_(new fftwf_complex[LongFftSize]),
      fftOutput_(new fftwf_complex[LongFftSize]),
      window_(LongFftSize),
      startTime_(std::chrono::steady_clock::now()),
      lastAnalyzeTime_(startTime_)
{
    for (size_t i = 0; i < LongFftSize; ++i)
        window_[i] = 0.5f - 0.5f * std::cos(2.0f * Pi * static_cast<float>(i) / static_cast<float>(LongFftSize - 1));
    fftPlan_ = fftwf_plan_dft_1d(static_cast<int>(LongFftSize), fftInput_.get(), fftOutput_.get(), FFTW_FORWARD, FFTW_MEASURE);
    reset();
}
...
void MusicAnalyzer::reset()
{
    ...
    quietSeconds_ = 0.0f;
    hardSilenceSeconds_ = 0.0f;
    resumeGraceSeconds_ = 0.0f;
...
float MusicAnalyzer::clamp01(float value)
{
...
float MusicAnalyzer::smooth(float previous, float target, float attackSeconds, float releaseSeconds, float dt)
{
...
std::vector<float> MusicAnalyzer::computeLongPowerSpectrum(const std::vector<float>& mono)
{
    ...
    constexpr float normalization = 4.0f / static_cast<float>(LongFftSize * LongFftSize);
...
std::array<float, MusicAnalyzer::FeatureBandCount> MusicAnalyzer::computeRawFeatureBands(const std::vector<float>& powerSpectrum,
                                                                                         double sampleRate) const
{
    std::array<float, FeatureBandCount> result{};
    const float resolution = static_cast<float>(sampleRate) / static_cast<float>(LongFftSize);
    for (size_t band = 0; band < FeatureRanges.size(); ++band) {
        const auto [low, high] = FeatureRanges[band];
        const size_t begin = std::clamp(static_cast<size_t>(low / resolution), size_t{1}, powerSpectrum.size() - 1);
        const size_t end = std::clamp(static_cast<size_t>(high / resolution) + 1, begin + 1, powerSpectrum.size());
        float sum = 0.0f;
...
void MusicAnalyzer::updateTempo(double nowSeconds, float onsetStrength)
{
    ...
    if (!onsetTimes_.empty() && nowSeconds - onsetTimes_.back() < 0.22) {
        if (onsetStrength <= onsetStrengths_.back())
            return;
    ...
    for (size_t i = 0; i < onsetStrengths_.size(); ++i) {
        if (onsetStrengths_[i] >= strongThreshold)
            strongIndices.push_back(i);
    }

    auto intervalFit = [](double interval, float period) {
        if (interval <= 0.0 || period <= 0.0f)
            return 0.0f;
    ...
    for (size_t bin = 0; bin < tempoHistogram_.size(); ++bin) {
        ...
        for (size_t i = 0; i < onsetTimes_.size(); ++i) {
            ...
            for (size_t j = i + 1; j < end; ++j) {
                ...
                if (interval < 0.20 || interval > 3.5)
                    continue;
                const float strength = std::sqrt(std::max(0.01f, onsetStrengths_[i] * onsetStrengths_[j]));
        ...
        for (size_t n = 1; n < strongIndices.size(); ++n) {
            ...
            if (interval < 0.28 || interval > 4.5)
                continue;
            const float strength = std::sqrt(std::max(0.01f, onsetStrengths_[a] * onsetStrengths_[b]));
    ...
    if (peakIt == tempoHistogram_.end() || *peakIt <= 0.05f)
        return;
    ...
    if (bpm >= 170.0f) {
        ...
        const int halfIndex =
            std::clamp(static_cast<int>(std::lround((halfBpm - 60.0f) * 2.0f)), 0, static_cast<int>(tempoHistogram_.size() - 1));
    ...
    if (octaveCorrected && !recentTempoEstimates_.empty() && percentile(recentTempoEstimates_, 0.50f) > bpm * 1.65f)
        recentTempoEstimates_.clear();
    if (replacedPulse && !recentTempoEstimates_.empty())
        recentTempoEstimates_.back() = bpm;
    else
        recentTempoEstimates_.push_back(bpm);
    ...
    for (size_t level = 0; level < thresholdQuantiles.size(); ++level) {
        ...
        for (size_t i = 1; i < indices.size(); ++i) {
            ...
            const double interval = onsetTimes_[b] - onsetTimes_[a];
            if (interval < 0.25 || interval > 4.5)
                continue;
            const float pairStrength = std::sqrt(std::max(0.01f, onsetStrengths_[a] * onsetStrengths_[b]));
    ...
    thresholdAgreement = thresholdWeight > 0.001f ? clamp01(thresholdAgreement / thresholdWeight) : 0.0f;
    ...
    const float targetConfidence =
        maturity * periodicity * (0.62f + 0.38f * consistency) * (0.72f + 0.28f * prominence) * (0.72f + 0.28f * thresholdAgreement);
    const float updateSeconds =
        onsetTimes_.size() >= 2 ? std::clamp(static_cast<float>(nowSeconds - onsetTimes_[onsetTimes_.size() - 2]), 0.01f, 0.5f) : 0.1f;
    ...
    if (smoothedBpm_ <= 1.0f) {
        ...
        const float target =
            (octaveCorrected || confirmedTempoChange) ? bpm : (maturity > 0.55f && consistency > 0.78f ? recentCenter : bpm);
        const float alpha = ((octaveCorrected || confirmedTempoChange) ? 0.34f : 0.08f) + beatConfidence_ * 0.24f +
                            (maturity > 0.75f && consistency > 0.90f ? 0.08f : 0.0f);
        smoothedBpm_ += (target - smoothedBpm_) * std::clamp(alpha, 0.06f, 0.48f);
    }
}

AudioProtocol::Frame MusicAnalyzer::analyze(const AudioRecorder::CapturedAudioFrame& audio, const std::vector<float>& displayBands)
{
    AudioProtocol::Frame frame;
    if (audio.mono.size() < LongFftSize)
        return frame;
    ...
    const bool hardSilence = rms < 0.00035f;
    if (hardSilence) {
        hardSilenceSeconds_ += dt;
        if (hardSilenceSeconds_ >= 0.22f) {
            wasSustainedSilence_ = true;
            dropArmed_ = false;
            quietSeconds_ = 0.0f;
        }
        if (hardSilenceSeconds_ >= 0.50f)
            lastBeatTime_ = -1000.0;
    }
    else {
        if (wasSustainedSilence_) {
            // Resume audio gently. The onset stream still feeds tempo tracking,
            // but consumers do not see the first transient as a drop/beat.
            resumeGraceSeconds_ = 0.75f;
            wasSustainedSilence_ = false;
        }
        hardSilenceSeconds_ = 0.0f;
    }
    resumeGraceSeconds_ = std::max(0.0f, resumeGraceSeconds_ - dt);
    const bool reacquiringAfterSilence = resumeGraceSeconds_ > 0.0f;
    ...
    for (size_t i = 0; i < FeatureBandCount; ++i) {
        ...
        if (rawDb < bandFloorDb_[i])
            bandFloorDb_[i] = rawDb;
        else
            bandFloorDb_[i] += std::min(dt * 0.55f, rawDb - bandFloorDb_[i]);
        if (rawDb > bandPeakDb_[i])
            bandPeakDb_[i] = rawDb;
        else
            bandPeakDb_[i] -= dt * 2.2f;
        bandPeakDb_[i] = std::max(bandPeakDb_[i], bandFloorDb_[i] + 20.0f);
        const float normalized = clamp01((rawDb - bandFloorDb_[i]) / (bandPeakDb_[i] - bandFloorDb_[i]));
    ...
    double logPower = 0.0;
    const float resolution = static_cast<float>(audio.sampleRate) / static_cast<float>(LongFftSize);
    ...
    const float geometricMean = std::exp(static_cast<float>(logPower / static_cast<double>(std::max<size_t>(1, power.size() - 1))));
    ...
    for (size_t i = 0; i < power.size(); ++i) normalizedSpectrum[i] = std::sqrt(power[i] / normalization);
    float flux = 0.0f;
    ...
    const float fluxThreshold =
        fluxMedian + (1.8f / std::max(0.25f, static_cast<float>(config_.transientSensitivity))) * std::max(fluxMad, 0.00002f);
    const float onsetStrength = clamp01((flux - fluxThreshold) / std::max(fluxThreshold * 2.0f, 0.00005f));
    const bool onsetEvent = onsetStrength > 0.16f && nowSeconds - lastOnsetTime_ > 0.065;

    const float kickTarget = clamp01(
        onsetStrength * 1.7f *
        (std::max(0.0f, bands[0] - previousBands_[0]) * 1.6f + std::max(0.0f, bands[1] - previousBands_[1]) * 1.2f + bands[1] * 0.35f));
    const float snareTarget =
        clamp01(onsetStrength * 1.5f *
                (std::max(0.0f, bands[2] - previousBands_[2]) * 0.8f + std::max(0.0f, bands[3] - previousBands_[3]) * 1.0f +
                 std::max(0.0f, bands[4] - previousBands_[4]) * 0.9f));
    const float hihatTarget =
        clamp01(onsetStrength * 2.0f *
                (std::max(0.0f, bands[5] - previousBands_[5]) * 1.15f + std::max(0.0f, bands[6] - previousBands_[6]) * 1.35f));
    ...
    if (onsetEvent) {
        ...
        if (!reacquiringAfterSilence) {
            ++onsetCounter_;
            frame.flags |= AudioProtocol::OnsetEvent;
        }

        // Tempo acquisition continues during resume grace, but visual event
        // counters stay quiet until the stream has settled.
    ...
    if (!onsetEvent && lastOnsetTime_ > -100.0 && nowSeconds - lastOnsetTime_ > staleTempoAfter) {
    ...
    const float predictedPhase = period > 0.0f ? std::fmod(static_cast<float>(std::max(0.0, sinceBeat)) / period, 1.0f) : 0.0f;
    const bool strongPercussiveOnset =
        onsetEvent && std::max(kickTarget, snareTarget * 0.72f) > 0.14f / std::max(0.3f, static_cast<float>(config_.beatSensitivity));
    ...
    const bool predictedBeat = beatConfidence_ > 0.62f && tempoStability_ > 0.50f && loudness > 0.08f && sinceBeat >= period * 0.98;
    const bool beatEvent = !reacquiringAfterSilence && (alignedOnset || predictedBeat);
    const float beatStrength = clamp01(std::max({kickEnvelope_, onsetStrength * 0.72f, predictedBeat ? beatConfidence_ * 0.5f : 0.0f}));
    if (beatEvent) {
    ...
    if (!reacquiringAfterSilence) {
        if (kickTarget > 0.22f)
            frame.flags |= AudioProtocol::KickEvent;
        if (snareTarget > 0.20f)
            frame.flags |= AudioProtocol::SnareEvent;
        if (hihatTarget > 0.16f)
            frame.flags |= AudioProtocol::HihatEvent;
    }

    const float energyTrend = std::clamp((fastLoudness_ - slowLoudness_) * 2.6f, -1.0f, 1.0f);
    const bool musicalLowEnergy = !hardSilence && loudness < 0.28f && bands[1] < 0.34f;
    if (musicalLowEnergy)
        quietSeconds_ += dt;
    else if (hardSilence)
        quietSeconds_ = 0.0f;
    else
        quietSeconds_ = std::max(0.0f, quietSeconds_ - dt * 1.4f);

    // A drop is a return from a *musical* low-energy passage. Sustained/digital
    // silence (pause, device switch, app reconnect) must never arm it. When the
    // tempo tracker is trustworthy, also require the low-energy passage to be
    // materially longer than one normal beat period; otherwise slow songs would
    // classify every kick after the inter-beat gap as a drop.
    const float dropQuietFloor = (beatConfidence_ > 0.45f && tempoStability_ > 0.40f) ? std::clamp(period * 1.35f, 0.58f, 1.35f) : 0.72f;
    if (!reacquiringAfterSilence && quietSeconds_ >= dropQuietFloor && quietSeconds_ <= 2.6f)
        dropArmed_ = true;
    if (hardSilence || reacquiringAfterSilence || quietSeconds_ > 2.6f)
        dropArmed_ = false;
    const bool dropEvent = dropArmed_ && nowSeconds - lastDropTime_ > 2.0 && onsetStrength > 0.28f && bands[1] > 0.60f &&
                           fastLoudness_ > 0.60f && energyTrend > 0.12f;
    if (dropEvent) {
        dropArmed_ = false;
        quietSeconds_ = 0.0f;
    ...
    const float stereoBalance = std::clamp((rightRms - leftRms) / std::max(leftRms + rightRms, 1.0e-7f), -1.0f, 1.0f);
    const float stereoCorrelation = std::clamp(cross / std::sqrt(std::max(leftSq * rightSq, 1.0e-12f)), -1.0f, 1.0f);

    const bool silence = hardSilence;
    if (silence)
        frame.flags |= AudioProtocol::Silent;
    ...
    frame.set(AudioProtocol::Feature::BeatPhase, lastBeatTime_ > -100.0 && period > 0.0f
                                                     ? std::fmod(static_cast<float>(nowSeconds - lastBeatTime_) / period, 1.0f)
                                                     : predictedPhase);
    ...
    for (size_t i = 0; i < WaveformPoints; ++i) {
        ...
        for (size_t sample = begin; sample < end; ++sample)
            if (std::abs(audio.mono[sample]) > std::abs(selected))
                selected = audio.mono[sample];
