...
{
    displayMode_->label("Display mode").description("Choose bars, radial layouts, waveform or scrolling spectrogram.").group("Layout");
    ...
    mirrorLayerBrightness_->label("Mirror backdrop brightness")
        .description(
            "Brightness of the mirrored rear layer. Lower values keep overlapping bars readable with monochrome palettes such as "
            "white-on-white.")
        .group("Bars")
        .visible_if("mirror_display", true)
        .step(0.05);
    fallingDots_->label("Peak markers").description("Show falling or outward-moving peak markers.").group("Bars");
    dotFallSpeed_->label("Peak fall speed")
        .description("How quickly peak markers return after a transient.")
        .group("Bars")
        .visible_if("falling_dots", true)
        .step(0.05);
    ...
    rotationSpeed_->label("Rotation speed")
        .description("Speed of radial layout rotation.")
        .group("Radial")
        .visible_if("rotate_visualization", true)
        .step(0.05);
    rainbow_->label("Rainbow palette").description("Color bands by frequency instead of using one base color.").group("Color");
    musicalColor_->label("Musical color shifts")
        .description("Shift hue with spectral centroid, stereo balance and song sections.")
        .group("Color")
        .visible_if("rainbow_colors", true);
    baseColor_->label("Base color")
        .description("Low-frequency/base color when rainbow mode is disabled.")
        .group("Color")
        .visible_if("rainbow_colors", false);
    accentColor_->label("Accent color")
        .description("High-frequency and transient accent color; keeps layered bars readable even with a white base.")
        .group("Color")
        .visible_if("rainbow_colors", false);
    percussionColor_->label("Percussion color accents")
        .description("Let kick, snare and hi-hat subtly shift color emphasis instead of only changing brightness.")
        .group("Color");
    ...
    waveformGain_->label("Waveform gain")
        .description("Vertical gain for the waveform without changing spectrum sensitivity.")
        .group("Waveform")
        .step(0.05);
    ...
    waveformSmoothing_->label("Waveform motion smoothing")
        ...
        .group("Waveform")
        .step(0.02);
    waveformThickness_->label("Waveform thickness")
        .description("Trace thickness in pixels. Higher values are brighter but cost slightly more fill work.")
        .group("Waveform");
    stereoMotion_->label("Stereo motion")
        .description("Let stereo balance move radial centers and width open the geometry.")
        .group("Response");

    add_property(barWidth_);
    add_property(gapWidth_);
    add_property(mirror_);
    add_property(mirrorLayerBrightness_);
    add_property(rainbow_);
    add_property(musicalColor_);
    add_property(baseColor_);
    add_property(accentColor_);
    add_property(percussionColor_);
    add_property(fallingDots_);
    add_property(dotFallSpeed_);
    add_property(displayMode_);
    add_property(circleRadius_);
    add_property(rotate_);
    add_property(rotationSpeed_);
    add_property(sensitivity_);
    add_property(smoothing_);
    add_property(releaseSpeed_);
    add_property(beatPulseEnabled_);
    add_property(showWaveform_);
    add_property(waveformStyle_);
    add_property(waveformGain_);
    add_property(waveformStabilization_);
    add_property(waveformSmoothing_);
    add_property(waveformThickness_);
    add_property(stereoMotion_);
...
void AudioSpectrumScene::updateSpectrum(const AudioState::Snapshot& audio, float dt)
{
    ...
    for (size_t i = 0; i < audio.spectrum.size(); ++i) {
        ...
        if (target > smoothed_[i])
            smoothed_[i] += (target - smoothed_[i]) * attack;
        else
            smoothed_[i] = std::max(target, smoothed_[i] - releaseSpeed_->get() * dt * 0.16f);
...
void AudioSpectrumScene::updateWaveform(const AudioState::Snapshot& audio, float dt)
{
    ...
    for (size_t x = 0; x < width; ++x) {
        const float sourcePosition = width <= 1 || sourceCount <= 1
                                         ? 0.0f
                                         : static_cast<float>(x) * static_cast<float>(sourceCount - 1) / static_cast<float>(width - 1);
        ...
        waveformScratch_[x] = std::clamp(audio.waveform[lo] + (audio.waveform[hi] - audio.waveform[lo]) * fraction, -1.0f, 1.0f);
    }
    ...
    if (waveformStabilization_->get() && width > 3) {
        ...
        if (previousEnergy > 0.0001f && targetEnergy > 0.0001f) {
            ...
            for (size_t shift = 0; shift < width; ++shift) {
                float dot = 0.0f;
                for (size_t x = 0; x < width; ++x) dot += waveformSmoothed_[x] * waveformTarget_[(x + shift) % width];
            ...
            if (bestCorrelation > 0.18f && bestShift != 0) {
                ...
                for (size_t x = 0; x < width; ++x) waveformTarget_[x] = waveformScratch_[(x + bestShift) % width];
            }
        }
    }
    ...
    for (size_t x = 0; x < width; ++x) waveformSmoothed_[x] += (waveformTarget_[x] - waveformSmoothed_[x]) * blend;
}

void AudioSpectrumScene::colorFor(float position, float intensity, const AudioState::Snapshot& audio, uint8_t& r, uint8_t& g,
                                  uint8_t& b) const
{
    ...
    const float transientLift =
        percussionColor_->get() ? kick * (1.0f - position) * 0.10f + snare * 0.07f + hihat * position * 0.12f : 0.0f;
    intensity = std::clamp(intensity * (1.0f + beatPulse_ * 0.10f) + transientLift, 0.0f, 1.0f);
    ...
    if (!rainbow_->get()) {
        ...
        if (percussionColor_->get())
            mix = std::clamp(mix + snare * 0.10f + hihat * 0.12f, 0.0f, 1.0f);
    ...
    if (musicalColor_->get()) {
        ...
        if (percussionColor_->get())
            hue += kick * -18.0f + snare * 22.0f + hihat * 34.0f;
    }
    const float saturation = std::clamp(0.78f + hihat * 0.14f - kick * 0.05f, 0.55f, 0.96f);
    color::hsv_to_rgb(hue, saturation, intensity, r, g, b);
}

void AudioSpectrumScene::renderBars(rgb_matrix::FrameCanvas* canvas, const AudioState::Snapshot& audio)
{
    const int barWidth = std::max(1, barWidth_->get());
    const int stride = std::max(1, barWidth + gapWidth_->get());
    const int count = std::min<int>(smoothed_.size(), std::max(1, matrix_width / stride));
    const auto mode = displayMode_->get().get();
    const int centerTop = (matrix_height - 1) / 2;
    const int centerBottom = matrix_height / 2;
    const int centerReach = std::max(1, std::min(centerTop + 1, matrix_height - centerBottom));

    auto sourceIndexFor = [&](int displayIndex) -> size_t {
        if (count <= 1 || smoothed_.size() <= 1)
            return 0;
        return std::min(smoothed_.size() - 1,
                        static_cast<size_t>(std::lround(static_cast<double>(displayIndex) * static_cast<double>(smoothed_.size() - 1) /
    ...
    auto drawNormalLayerBar = [&](int i, bool mirrored, float brightness) {
        ...
        const float colorPosition = count <= 1 ? 0.0f : static_cast<float>(i) / static_cast<float>(count - 1);
        const int x = i * stride;
        const int barHeight = std::clamp(static_cast<int>(std::lround(value * matrix_height)), 0, matrix_height);

        for (int row = 0; row < barHeight; ++row) {
            const int y = matrix_height - 1 - row;
            const float intensity = 0.4f + 0.6f * static_cast<float>(row + 1) / static_cast<float>(std::max(1, barHeight));
            uint8_t r, g, b;
        ...
        if (fallingDots_->get() && peak > 0.001f) {
            const int peakY =
                std::clamp(matrix_height - 1 - static_cast<int>(std::lround(peak * (matrix_height - 1))), 0, matrix_height - 1);
            uint8_t r, g, b;
    ...
    if (mode == DisplayMode::NORMAL) {
        ...
        if (mirror_->get()) {
            ...
            for (int i = 0; i < count; ++i) drawNormalLayerBar(i, true, rearBrightness);
        }
        for (int i = 0; i < count; ++i) drawNormalLayerBar(i, false, 1.0f);
        return;
    }
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (mode == DisplayMode::CENTER_OUT) {
            const int extent = std::clamp(static_cast<int>(std::lround(value * centerReach)), 0, centerReach);

            for (int distance = 0; distance < extent; ++distance) {
                const float intensity = 0.35f + 0.65f * static_cast<float>(distance + 1) / static_cast<float>(std::max(1, extent));
                uint8_t r, g, b;
                colorFor(colorPosition, intensity, audio, r, g, b);
                ...
                for (int w = 0; w < barWidth; ++w) {
                    ...
                    if (lowerY < matrix_height)
                        canvas->SetPixel(px, lowerY, r, g, b);
                }
            }

            if (fallingDots_->get() && peak > 0.001f) {
                const int peakDistance = std::clamp(static_cast<int>(std::lround(peak * (centerReach - 1))), 0, centerReach - 1);
...
void AudioSpectrumScene::renderCircle(rgb_matrix::FrameCanvas* canvas, const AudioState::Snapshot& audio, bool spiral)
{
    const float stereoBalance = stereoMotion_->get() ? stereoBalanceResponse_ : 0.0f;
    const float stereoWidth = stereoMotion_->get() ? stereoWidthResponse_ : 0.0f;
    const float cx = matrix_width * (0.5f + stereoBalance * 0.08f);
    const float cy = matrix_height * 0.5f;
    const float size = static_cast<float>(std::min(matrix_width, matrix_height));
    const float baseRadius = size * 0.21f * circleRadius_->get() * (1.0f + radialKick_ * 0.045f + beatPulse_ * 0.025f);
    const float maxLength = size * 0.30f;

    for (size_t i = 0; i < smoothed_.size(); ++i) {
        const float t = static_cast<float>(i) / static_cast<float>(std::max<size_t>(1, smoothed_.size() - 1));
        const float angle = rotation_ + t * 2.0f * Pi * (spiral ? 2.4f : 1.0f);
        const float radius = baseRadius * (spiral ? 0.35f + t * (1.5f + stereoWidth * 0.28f) : 1.0f + stereoWidth * 0.08f);
        const float length = std::clamp(smoothed_[i], 0.0f, 1.0f) * maxLength;

        for (float distance = 0.0f; distance <= length; distance += 0.7f) {
            const int x = static_cast<int>(std::round(cx + std::cos(angle) * (radius + distance)));
            const int y = static_cast<int>(std::round(cy + std::sin(angle) * (radius + distance)));
            uint8_t r, g, b;
            colorFor(t, 0.28f + 0.72f * distance / std::max(1.0f, length), audio, r, g, b);
        ...
        if (fallingDots_->get() && i < peaks_.size() && peaks_[i] > 0.001f) {
            const float peakRadius = radius + std::clamp(peaks_[i], 0.0f, 1.0f) * maxLength;
            ...
            for (int width = -1; width <= 1; ++width) {
                const int x = static_cast<int>(std::round(cx + std::cos(angle) * peakRadius + tangentX * width));
                const int y = static_cast<int>(std::round(cy + std::sin(angle) * peakRadius + tangentY * width));
                addPixel(canvas, x, y, r, g, b);
            }
        }
    }
...
void AudioSpectrumScene::renderWaveform(rgb_matrix::FrameCanvas* canvas, const AudioState::Snapshot& audio)
{
    if (waveformSmoothed_.empty())
        return;

    const bool standalone = displayMode_->get().get() == DisplayMode::WAVEFORM;
    const float balance = stereoMotion_->get() ? std::clamp(audio.feature(AudioProtocol::Feature::StereoBalance), -1.0f, 1.0f) : 0.0f;
    ...
    const float amplitude =
        matrix_height * (standalone ? 0.44f : 0.31f) * waveformGain_->get() * (0.86f + loudness * 0.10f + beatPulse_ * 0.08f);
    const int thickness = std::clamp(waveformThickness_->get(), 1, 3);
    const auto style = waveformStyle_->get().get();

    auto sampleForX = [&](int x) {
        const size_t index = std::min(waveformSmoothed_.size() - 1, static_cast<size_t>(std::max(0, x)));
    ...
    auto drawTracePoint = [&](int x, int y, float position, float intensity) {
        ...
        for (int offset = -(thickness - 1); offset <= thickness - 1; ++offset) {
            ...
            addPixel(canvas, x, y + offset, static_cast<uint8_t>(r * falloff), static_cast<uint8_t>(g * falloff),
    ...
    for (int x = 0; x < matrix_width; ++x) {
        ...
        if (style == WaveformStyle::FILLED) {
            ...
            for (int py = from; py <= to; ++py) drawTracePoint(x, py, position, standalone ? 0.96f : 0.68f);
        }
        previousY = y;
    }
}

void AudioSpectrumScene::renderSpectrogram(rgb_matrix::FrameCanvas* canvas, const AudioState::Snapshot& audio)
{
    for (int x = 0; x < matrix_width && x < static_cast<int>(history_.size()); ++x) {
        const auto& column = history_[x];
        for (int y = 0; y < matrix_height; ++y) {
            const size_t index =
                std::min(column.size() - 1, static_cast<size_t>((1.0f - static_cast<float>(y) / matrix_height) * (column.size() - 1)));
            const float value = std::clamp(column[index] * (1.0f - x / static_cast<float>(matrix_width) * 0.45f), 0.0f, 1.0f);
            if (value < 0.025f)
                continue;
            uint8_t r, g, b;
            colorFor(static_cast<float>(index) / column.size(), value, audio, r, g, b);
...
bool AudioSpectrumScene::render(rgb_matrix::FrameCanvas* canvas)
{
    ...
    canvas->Clear();
    if (!audio.fresh() || audio.spectrum.empty())
        return false;
    ...
    const float rawKick = std::clamp(audio.feature(AudioProtocol::Feature::Kick), 0.0f, 1.0f);
    radialKick_ = smoothResponse(radialKick_, rawKick, 0.025f, 0.20f, dt);
    stereoBalanceResponse_ = smoothResponse(
        stereoBalanceResponse_, std::clamp(audio.feature(AudioProtocol::Feature::StereoBalance), -1.0f, 1.0f), 0.12f, 0.28f, dt);
    stereoWidthResponse_ =
        smoothResponse(stereoWidthResponse_, std::clamp(audio.feature(AudioProtocol::Feature::StereoWidth), 0.0f, 1.0f), 0.16f, 0.34f, dt);
    tempoRateResponse_ = smoothResponse(tempoRateResponse_, tempoRate(audio), 0.55f, 0.85f, dt);
    ...
    if (!beatPrimed_) {
        ...
        if (audio.beat_counter < lastBeat_) {
            lastBeat_ = audio.beat_counter;
        }
        else if (beatPulseEnabled_->get() && audio.beat_counter > lastBeat_) {
            beatPulse_ = std::max(beatPulse_, 0.72f);
            lastBeat_ = audio.beat_counter;
        }
    }
    if (beatPulseEnabled_->get())
        beatPulse_ *= std::exp(-dt / 0.22f);
    else
        beatPulse_ = 0.0f;

    if (rotate_->get()) {
        const float snare = std::clamp(audio.feature(AudioProtocol::Feature::Snare), 0.0f, 1.0f);
        rotation_ += dt * rotationSpeed_->get() * (0.42f + tempoRateResponse_ * 0.38f + snare * 0.08f);
    }

    switch (displayMode_->get().get()) {
        case DisplayMode::CIRCLE:
            renderCircle(canvas, audio, false);
            break;
        case DisplayMode::SPIRAL:
            renderCircle(canvas, audio, true);
            break;
        case DisplayMode::WAVEFORM:
            renderWaveform(canvas, audio);
            break;
        case DisplayMode::SPECTROGRAM:
            renderSpectrogram(canvas, audio);
            break;
        default:
            renderBars(canvas, audio);
            break;
    }
...
Scenes::SceneDescriptor Scenes::AudioSpectrumScene::get_descriptor() const
{
    ...
    d.performance_cost = 0.58f;
    d.variants = {
        {"bars",
         "Layered bars",
         "Balanced spectrum bars with mirrored depth and peak motion",
        ...
        {"waveform",
         "Smooth waveform",
         "A clean stabilized trace instead of individual spectrum bars",
         {{"display_mode", "WAVEFORM"},
          {"waveform_style", "TRACE"},
          {"waveform_stabilization", true},
          {"waveform_smoothing", 0.90f},
          {"waveform_thickness", 1},
          {"stereo_motion", true}},
         {"music", "waveform", "minimal"},
         0.52f,
         0.56f,
         1.0f,
         0.42f},
        {"radial",
         "Radial spectrum",
         "Circular spectrum with slow rotation and beat pulse",
         {{"display_mode", "CIRCLE"},
          {"circle_radius", 0.70f},
          {"rotate_visualization", true},
        ...
        {"spectrogram",
         "Spectrogram",
         "Scrolling frequency history for a denser analytical texture",
