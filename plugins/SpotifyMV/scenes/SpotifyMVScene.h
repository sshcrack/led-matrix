...
#include "matrix/SpotifyMVPlugin.h"
...
public:
  ...
  void prepare_runtime(const RuntimeInputs::Snapshot& snapshot) override;
  ...
  SceneCapabilities get_capabilities() const override {
    ...
    // SpotifyMV already receives desktop-produced video frames. The worker
    // cannot and should not proxy that stream a second time.
