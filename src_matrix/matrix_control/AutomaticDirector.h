...
public:
    ...
    [[nodiscard]] std::vector<Candidate> rank(const std::vector<std::shared_ptr<Scenes::Scene>>& scenes,
                                              const RuntimeInputs::Snapshot& runtime_inputs, const std::string& exclude_name = "") const;

    Decision choose(const std::vector<std::shared_ptr<Scenes::Scene>>& scenes, const RuntimeInputs::Snapshot& runtime_inputs,
...
private:
    ...
    [[nodiscard]] float history_multiplier(const std::string& scene, const std::string& family) const;
};
