...
* **`parallel_deterministic` (Global bool): **Optional**. If true, the scene will be rendered in parallel across multiple threads, but must produce the same output regardless of execution order (e.g., no random number generation or mutable global state). Default is true.
...
* **Math.atan2 not existant:** Use Math.atan with two arguments instead.
