...
std::unique_ptr<Server::router_t> Server::add_schedule_routes(std::unique_ptr<router_t> router) {
    ...
    router->http_get("/scheduling_status", [](auto req, auto) {
    ...
    router->http_get("/set_scheduling_enabled", [](auto req, auto) {
        const auto qp = restinio::parse_query(req->header().query());
        if (!qp.has("enabled")) {
            return reply_with_error(req, "No enabled parameter given");
        }

        const bool enabled = qp["enabled"] == "true";
