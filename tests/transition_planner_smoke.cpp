...
public:
    ...
    Scenes::SceneDescriptor get_descriptor() const override {
        auto d=Scene::get_descriptor(); d.family=family_; d.intensity=intensity_; d.motion=motion_; return d;
    }
    tmillis_t get_default_duration() override { return 1000; }
    int get_default_weight() override { return 1; }
private:
    std::string name_, family_; float intensity_, motion_;
};
AudioState::Snapshot music(float phase, bool drop=false) {
    AudioState::Snapshot a; a.available=true; a.age_seconds=0.01f;
    ...
    if(drop) a.set(AudioProtocol::Feature::Drop, 1.f);
    return a;
}
}
int main() {
    ...
    if(!plan.beat_synchronized || plan.start_delay_ms < 200 || plan.start_delay_ms > 300 || plan.duration_ms < 450 || plan.duration_ms > 550) {
        std::cerr << "beat plan was not quantized around 120 BPM\n"; return 1;
    }
    auto drop=planner.plan(calm,peak,750,"blend",music(.2f,true));
    if(drop.name != "glitch_cut" || drop.start_delay_ms != 0 || drop.duration_ms > 300) {
        std::cerr << "drop transition was not immediate/high-energy\n"; return 2;
    }
