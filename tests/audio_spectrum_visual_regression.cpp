...
float changed_pixel_fraction(const std::vector<std::uint8_t> &a,
...
AudioProtocol::Frame waveform_frame(std::uint32_t sequence, float phase) {
    ...
    for (std::size_t i = 0; i < frame.waveform.size(); ++i) {
        ...
        frame.waveform[i] = std::sin((x * 4.0f + phase) * 2.0f * Pi) * 0.72f +
                            std::sin((x * 9.0f + phase * 1.7f) * 2.0f * Pi) * 0.18f;
    }
    return frame;
}
}
...
int main() {
    ...
    if (foregroundLuma < 180 || rearLuma <= 0 ||
        foregroundLuma < static_cast<int>(rearLuma * 1.6f)) {
