...
#
# Records animated GIF previews only for scenes that preview_gen cannot render
# itself (can_generate_preview == false), such as VideoScene, SpotifyMV and
# ShadertoyScene.
#
# Prerequisites:
...
#                        If omitted, all scenes that cannot use preview_gen are captured.
...
#
# Workflow:
#   1. Generate all previews supported by preview_gen:
...
# ---------------------------------------------------------------------------
# Get the list of scenes that cannot be generated by preview_gen
# ---------------------------------------------------------------------------
...
for s in scenes:
    ...
    if not caps.get('can_generate_preview', not s.get('needs_desktop', False)):
        print(s['name'])
"
    )
fi
...
if [[ ${#TARGET_SCENES[@]} -eq 0 ]]; then
    echo "All scenes support preview_gen. Nothing requires manual capture."
    exit 0
fi
