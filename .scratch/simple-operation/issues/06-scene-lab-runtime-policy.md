...
Final review separated Scene Lab session identity from per-update generation. All mutating/lease routes now require the stable session id, so an older browser tab cannot heartbeat, stop, update, or save against a replacement session while legitimate same-session updates may advance generation concurrently. Covered by runtime smoke tests and an HTTP multi-session integration check.
