...
export default function SceneGallery() {
  ...
  const filtered = useMemo(() => (scenes ?? []).filter(s =>
    (category === 'All' || (s.category ?? 'General') === category) && s.name.toLowerCase().includes(query.trim().toLowerCase())
  ...
  const addToPreset = async () => {
    ...
    try {
      ...
      const args = Object.fromEntries(selected.properties.map(prop => [prop.name, prop.default_value]))
      const scene: Scene = { uuid: uuidv4(), type: selected.name, arguments: args }
      ...
      <section className="glass-panel min-w-0 overflow-hidden rounded-2xl">
        ...
        <div className="max-h-[calc(100vh-285px)] overflow-y-auto p-4">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{filtered.map(scene => {
            ...
            return <button key={scene.name} data-selected={active} className="scene-tile" onClick={() => setSelectedName(scene.name)}>
              ...
              <div className="p-3"><div className="truncate text-sm font-semibold">{scene.name}</div><div className="mt-1 flex justify-between text-[11px] text-muted-foreground"><span>{scene.category}</span><span className="flex items-center gap-1"><SlidersHorizontal className="h-3 w-3" />{scene.properties.length}</span></div></div>
            </button>})}</div>}
        </div>
      </section>
